...
const API = {
  ...
  overview()          { return this.useBackend ? this._try("/api/overview")        : Promise.resolve(LocalAPI.overview()); },
  machine(id)         { return this.useBackend ? this._try(`/api/machines/${id}`)  : Promise.resolve(LocalAPI.machine(id)); },
  history(id, n = 60) { return this.useBackend ? this._try(`/api/machines/${id}/history?n=${n}`) : Promise.resolve(LocalAPI.history(id, n)); },
  alerts()            { return this.useBackend ? this._try("/api/alerts")          : Promise.resolve(LocalAPI.alerts()); },
  meta()              { return this.useBackend ? this._try("/api/meta")            : Promise.resolve(LocalAPI.meta()); },
  twin(id, action)    { return this.useBackend ? this._post(`/api/machines/${id}/twin`, { action }) : Promise.resolve(LocalAPI.twin(id, action)); },
  inject(id, fault)   { return this.useBackend ? this._post(`/api/machines/${id}/inject-fault`, { fault }) : Promise.resolve(LocalAPI.inject(id, fault)); },
  repair(id)          { return this.useBackend ? this._post(`/api/machines/${id}/repair`, {}) : Promise.resolve(LocalAPI.repair(id)); },
  power(id, running)  { return this.useBackend ? this._post(`/api/machines/${id}/power`, { running }) : Promise.resolve(LocalAPI.power(id, running)); },
};
...
const state = { view: "overview", selectedMachine: "M-001", twinMachine: "M-002", meta: null };
...
const VIEW_META = {
  ...
  about:    ["About the Project", "MachineSense AI — MSME Idea Hackathon"],
};
...
async function renderOverview() {
  ...
  $("#overviewMachines").innerHTML = o.machines.map(m => machineCard(m)).join("");
  o.machines.forEach(m => healthGauge($(`#g-${m.id}`), m.health, { size: 68 }));
  $$("#overviewMachines .mcard").forEach(c => c.addEventListener("click", () => {
    state.selectedMachine = c.dataset.id; switchView("machines");
  }));
  updateAlertBadge(o.active_alerts);
}

function machineCard(m) {
  return `<div class="card mcard" data-id="${m.id}">
    <div class="mhead">
      <div>
        <div class="mname">${m.name}</div>
        <div class="mmeta">${m.id} · ${m.type} · ${m.location}</div>
      </div>
      <div class="gauge-wrap"><canvas id="g-${m.id}"></canvas></div>
    </div>
    <div class="mfoot">
      <span class="chip ${m.band}"><span class="cd"></span>${cap(m.band)}</span>
      ${m.fault ? `<span class="chip critical">⚠ ${m.fault}</span>` : ""}
...
async function renderMachine(id) {
  const o = await API.overview();
  const options = o.machines.map(m => `<option value="${m.id}" ${m.id === id ? "selected" : ""}>${m.name} (${m.id})</option>`).join("");
  const d = await API.machine(id);
  const hist = await API.history(id, 60);

  const p = d.prediction;
  const rulLabel = p.rul_hours >= 1900 ? "Stable" : `${p.rul_days.toFixed(1)} days`;
  ...
  $("#machineDetail").innerHTML = `
    <div class="toolbar">
      ...
      ${d.fault ? `<span class="chip critical">⚠ ${d.fault}</span>` : ""}
    ...
    <div class="grid detail-grid">
      <div class="card chart-box">
        ...
        <div class="sub-note">Streaming at 0.5 Hz · thresholds shown as dashed lines per sensor scale</div>
      </div>

      <div class="card">
        <h3 class="ct">${icoGauge} Prediction</h3>
        <div class="mini">
          <div class="box"><div class="l">Remaining Life</div><div class="v">${rulLabel}</div></div>
          <div class="box"><div class="l">Failure Risk</div><div class="v" style="color:${riskColor(p.failure_probability)}">${Math.round(p.failure_probability * 100)}%</div></div>
        </div>
        <div class="mini" style="margin-top:10px">
          <div class="box"><div class="l">Health Trend</div><div class="v">${p.trend_per_hour} <small>/h</small></div></div>
          <div class="box"><div class="l">Confidence</div><div class="v">${Math.round(p.confidence * 100)}%</div></div>
        </div>
    ...
    <div class="grid detail-grid" style="margin-top:16px">
      ...
      <div class="card">
        ...
        <ul class="reasons">${d.explanation.reasons.map(r => `<li>${r}</li>`).join("")}</ul>
    ...
    <div class="card" style="margin-top:16px">
      ...
      <div id="recs">${d.recommendations.map(r => `
        <div class="rec">
          <span class="prio ${r.priority}">${r.priority}</span>
          <div><div class="ractn">${r.action}</div>
          <div class="sub-note" style="margin-top:3px">Driver: ${cap(r.driver)}</div></div>
        </div>`).join("")}</div>
    </div>`;

  // sensor bars
  $("#sensorBars").innerHTML = Object.entries(d.sensors).map(([s, c]) => {
  ...
  // contribution bars
  $("#contribBars").innerHTML = d.explanation.contributions.map(c => {
...
function drawSensorChart(hist) {
  ...
  const series = ["vibration", "temperature", "current", "power"].map(s => ({
    label: s, color: SENSOR_COLORS[s], data: pts.map(p => normalize(s, p[s])),
  }));
  lineChart($("#sensorChart"), series, { height: 210, min: 0, max: 100 });
...
function normalize(s, v) { const c = window.MS_SENSORS[s]; return Math.min(100, (v / c.critical) * 100); }
...
async function renderTwin() {
  const o = await API.overview();
  const options = o.machines.map(m => `<option value="${m.id}" ${m.id === state.twinMachine ? "selected" : ""}>${m.name} (${m.id})</option>`).join("");
  ...
  $("#twinView").innerHTML = `
    <div class="card" style="margin-bottom:16px">
      ...
      <div class="toolbar" style="margin:0">
        ...
        <select class="input" id="twinAction">${actions.map(a => `<option value="${a}">${actionLabels[a] || a}</option>`).join("")}</select>
...
async function runTwin() {
  const id = $("#twinMachine").value, action = $("#twinAction").value;
  state.twinMachine = id;
  const r = await API.twin(id, action);
  ...
  $("#twinResult").innerHTML = `
    ...
    <div class="card" style="margin-top:16px">
      ...
      <div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(180px,1fr))">
        ...
        ${impactBox("Remaining Life", (dR >= 0 ? "+" : "") + Math.round(dR / 16) + " days", dR >= 0)}
        ${impactBox("Failure Risk", Math.round((r.after.failure_probability - r.before.failure_probability) * 100) + "%", r.after.failure_probability <= r.before.failure_probability)}
...
function twinPanel(title, s, color) {
  return `<div class="card tw-panel">
    <h4>${title}</h4>
    <canvas id="tw-${title.includes("Before") ? "b" : "a"}" style="margin-bottom:8px"></canvas>
...
// draw gauges after twin panels are in the DOM
const _origRunTwin = runTwin;
runTwin = async function () {
  await _origRunTwin();
  const id = $("#twinMachine").value, action = $("#twinAction").value;
  const r = await API.twin(id, action);
  if ($("#tw-b")) healthGauge($("#tw-b"), r.before.health, { size: 100 });
  if ($("#tw-a")) healthGauge($("#tw-a"), r.after.health, { size: 100 });
};
...
async function renderEnergy() {
  ...
  $("#energyView").innerHTML = `
    <div class="grid kpis" style="margin-bottom:18px">
      ...
      <div class="card kpi"><div class="label">Recoverable Waste</div><div class="value" style="color:var(--good)">${fmtINR(savings)}</div><div class="sub">${Math.round(savings / totalCost * 100)}% of spend</div><div class="ico">${icoLeaf}</div></div>
  ...
  const maxPower = Math.max(...details.map(d => d.energy.avg_power_kw));
  $("#energyBars").innerHTML = details.map(d => {
    const e = d.energy, pct = (e.avg_power_kw / maxPower) * 100;
    const col = e.efficiency_pct >= 85 ? "var(--good)" : e.efficiency_pct >= 70 ? "var(--watch)" : "var(--warn)";
    return `<div class="sensor-row">
      <div class="sname" style="width:130px">${d.name}</div>
  ...
  $("#aboutView").innerHTML = `
    <div class="card" style="margin-bottom:16px">
      ...
      <p class="muted" style="font-size:13.5px;line-height:1.6;max-width:820px">
        ...
        and digital-twin simulation to help <b>MSMEs</b> reduce downtime, maintenance cost and energy consumption —
    ...
    <div class="card" style="margin-top:16px">
      ...
      <div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(150px,1fr))">
        ...
        <div class="mini" style="margin:0"><div class="box"><div class="l">Est. Project Cost</div><div class="v" style="font-size:15px">₹19.8 L</div></div></div>
        <div class="mini" style="margin:0"><div class="box"><div class="l">Market</div><div class="v" style="font-size:15px">63M+ MSMEs</div></div></div>
        <div class="mini" style="margin:0"><div class="box"><div class="l">Model</div><div class="v" style="font-size:15px">SaaS + Kit</div></div></div>
      </div>
      <div class="sub-note">MSME Idea Hackathon · Explainable Industrial Intelligence for Predictive Maintenance, Digital Twin & Energy Optimization</div>
    </div>`;
}
...
async function boot() {
  ...
  // Live refresh of the active view.
  setInterval(() => {
    if (state.view === "overview") renderOverview();
    else if (state.view === "machines") { const c = $("#sensorChart"); if (c) refreshMachineCharts(); }
    else if (state.view === "alerts") renderAlerts();
    else if (state.view === "energy") { /* periodic redraw handled on nav */ }
  }, 2500);
}
...
async function refreshMachineCharts() {
  ...
  drawSensorChart(hist); drawHealthChart(hist);
  // update sensor value bars + prediction without full re-render
  const bars = $("#sensorBars");
  if (bars) bars.innerHTML = Object.entries(d.sensors).map(([s, c]) => {
    const pct = Math.min(100, (c.value / c.critical) * 100);
    const col = c.value >= c.critical ? "var(--crit)" : c.value >= c.warn ? "var(--warn)" : "var(--good)";
    return `<div class="sensor-row"><div class="sname">${s}</div><div class="sval" style="color:${col}">${c.value}${c.unit}</div><div class="bar"><i style="width:${pct}%;background:${col}"></i></div></div>`;
  }).join("");
}

window.addEventListener("resize", () => { if (state.view === "machines") refreshMachineCharts(); });
boot();
